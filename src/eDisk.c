...
// **write this function**
	uint8_t *startAddr = (uint8_t *)(EDISK_ADDR_MIN + SECTOR_SIZE*sector);
...
// **write this function**
  uint32_t startAddr = (EDISK_ADDR_MIN + SECTOR_SIZE*sector);
	int successfulWrites;
	if ((EDISK_ADDR_MIN + SECTOR_SIZE*sector) > EDISK_ADDR_MAX){
		return RES_PARERR;
	}
	else{
		successfulWrites =Flash_WriteArray((uint32_t *)buff, startAddr, SECTOR_SIZE/4);	
		if (successfulWrites == SECTOR_SIZE/4){
			return RES_OK;
		}
		else{
			return RES_ERROR;
...
enum DRESULT eDisk_SectorFormat(uint8_t sector)
{
	uint32_t address;
	address = EDISK_ADDR_MIN + 1024*sector; //start of disk
